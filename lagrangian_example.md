...
**Problem.**  
Find the rectangle of maximum area with fixed perimeter \(P>0\).
...
\[
\frac{\partial\mathcal{L}}{\partial x} = y - \lambda = 0,\\[4pt]
\frac{\partial\mathcal{L}}{\partial y} = x - \lambda = 0,\\[4pt]
...
This illustrates the core Lagrange-multiplier procedure:

1. write the Lagrangian \(\mathcal{L}=f+\lambda(\text{constraint})\),
2. set partial derivatives to zero (stationarity),
3. solve together with the constraint,
4. verify the nature of the extremum (via substitution or second-derivative/concavity).
